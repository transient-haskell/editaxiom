...
-- mkdir -p ./static && ghcjs -itransient/src -itransient-universe/src  -iaxiom/src ${4} ${1} -o static/out && runghc -itransient/src -itransient-universe/src -iaxiom/src  ${4} ${1}  ${2} ${3}
module Main where
...
main= keep $ initNode $ onBrowser $ local $   buttons  <|> linksample
    ...
    radio :: TransIO ()
    radio= do
           r <- render $ getRadio [fromString v ++> setRadioActive False v
                         | v <- ["red","green","blue"]]
